...
def report_memory_usage_one_pp_rank(
    ...
    overhead_memory = getattr(args, 'overhead', 0) or 0
    total_memory = weight_grad_optim_memory + activation_memory + overhead_memory

    print(
        f"Theoretical memory footprints: weight and optimizer={weight_grad_optim_memory:.2f} GB, "
        f"activation={activation_memory:.2f} GB, overhead={overhead_memory:.2f} GB, total={total_memory:.2f} GB\n"
    )
...
def extra_args_provider(parser):
    parser.add_argument('--overhead', type=int, default=10, choices=[5, 10],
                        help='Overhead per GPU in GB (default: 10)')
    return parser
...
if __name__ == "__main__":
    ...
    # initialize_megatron(allow_no_cuda=True, skip_mpu_initialization=False)
